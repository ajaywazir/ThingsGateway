...
public partial class UpdateZipFilePage
{
    ...
    private async Task<QueryData<UpdateZipFile>> OnQueryAsync(QueryPageOptions options)
    {
        ...
        {
            var data = await UpdateZipFileHostedService.Page(options);
            return data;
        });
    }
    ...
    private async Task OnAdd(IEnumerable<UpdateZipFile> pluginOutputs)
    {
        ...
        {
            [nameof(SaveUpdateZipFile.OnSaveUpdateZipFile)] = new Func<UpdateZipFileAddInput, Task>(UpdateZipFileHostedService.SaveUpdateZipFile),
        });
