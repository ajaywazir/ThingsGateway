...
internal sealed class FileDownloadManager
{
    ...
    internal async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            int numBytesRead;
            while (!cancellationToken.IsCancellationRequested &&
                   (numBytesRead = await contentStream.ReadAsync(buffer, cancellationToken).ConfigureAwait(false)) > 0)
            {
                // 将读取的数据写入文件
                await fileStream.WriteAsync(buffer.AsMemory(0, numBytesRead), cancellationToken).ConfigureAwait(false);
                ...
                // 发送文件传输进度到通道
                await _progressChannel.Writer.WriteAsync(fileTransferProgress, cancellationToken).ConfigureAwait(false);
            }
        ...
        finally
        {
            ...
            await reportProgressTask.ConfigureAwait(false);
        }
    }
    ...
    internal async Task HandleProgressChangedAsync(FileTransferProgress fileTransferProgress)
    {
        ...
        if (FileTransferEventHandler is not null)
        {
            await DelegateExtensions.TryInvokeAsync(FileTransferEventHandler.OnProgressChangedAsync,
                fileTransferProgress).ConfigureAwait(false);
        }

        await _httpFileDownloadBuilder.OnProgressChanged.TryInvokeAsync(fileTransferProgress).ConfigureAwait(false);
    }
